...
def load_full_checkpoint(filepath, model, optimizer=None, scheduler=None, device='cuda'):
    ...
    print(f"Checkpoint loaded from {filepath} (epoch {checkpoint.get('epoch', 'unknown')})")
    return checkpoint
...
class MTLModel(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        exp_out = self.exp_layers(sequence_output).squeeze() * attention_mask
        cls_out = self.cls_layers(pooled_output)
        return cls_out, exp_out

class MTLTrainer:
    def __init__(self, args, data=None, cls_labels=None, exp_labels=None):
        self.args = args
        self.tokenizer = AutoTokenizer.from_pretrained(args.model_config, use_fast=False)
        if data is not None:
            self.data = np.array([self.tokenizer.cls_token + " " + x + " " + self.tokenizer.sep_token for x in data])
            self.exp_labels = exp_labels
            self.num_classes = int(np.max(cls_labels)) + 1
    ...
    def fit(self, input_ids, attention_masks, cls_labels, exp_labels, cls_criterion, exp_criterion, exp_weight,
            batch_size, accumulation_steps=1):
        self.model.train()
        total_loss = 0
        epoch_indices = random.sample(range(len(input_ids)), len(input_ids))
        batch_num = 0
        ...
        for batch_start in range(0, len(input_ids), batch_size):
            ...
            batch_input_ids = input_ids[batch_indices].to(self.args.device)
            batch_attention_masks = attention_masks[batch_indices].to(self.args.device)
            batch_cls_labels = cls_labels[batch_indices].to(self.args.device)
            batch_exp_labels = exp_labels[batch_indices].to(self.args.device)
            with torch.cuda.amp.autocast() if use_amp else nullcontext():
                cls_preds, exp_preds = self.model(batch_input_ids, batch_attention_masks)
                cls_loss = cls_criterion(cls_preds, batch_cls_labels).mean(dim=-1).sum()
                exp_loss = exp_criterion(exp_preds, batch_exp_labels.float()).mean(dim=-1).sum()
                loss = (cls_loss + exp_weight * exp_loss) / accumulation_steps
            if use_amp:
            ...
            batch_num += 1
            if (batch_num % accumulation_steps == 0) or (batch_end == len(input_ids)):
                if use_amp:
    ...
    def predict(self, input_ids, attention_masks, cls_labels=None, exp_labels=None, cls_criterion=None,
                exp_criterion=None, exp_weight=0.07, batch_size=128):
        self.model.eval()
        cls_preds = []
        exp_preds = []
        exp_probs = []
        ...
        with torch.no_grad():
            for batch_start in range(0, len(input_ids), batch_size):
                batch_end = min(batch_start + batch_size, len(input_ids))
                batch_input_ids = input_ids[batch_start:batch_end].to(self.args.device)
                batch_attention_masks = attention_masks[batch_start:batch_end].to(self.args.device)
                cls_outs, exp_outs = self.model(batch_input_ids, batch_attention_masks)
                if cls_labels is not None:
                    batch_cls_labels = cls_labels[batch_start:batch_end].to(self.args.device)
                    batch_exp_labels = exp_labels[batch_start:batch_end].to(self.args.device)
                    cls_loss = cls_criterion(cls_outs, batch_cls_labels).mean(dim=-1).sum()
                    exp_loss = exp_criterion(exp_outs, batch_exp_labels.float()).mean(dim=-1).sum()
    ...
    def eval(self, train_indices=None, valid_indices=None, test_indices=None):
        ...
        self.optimizer = AdamW(self.model.parameters(), lr=self.args.lr, eps=1e-8, weight_decay=weight_decay)
        ...
        if hasattr(self.args, 'class_weights') and self.args.class_weights is not None:
            ...
            cls_criterion = torch.nn.CrossEntropyLoss(weight=class_weights, reduction='none')
            print("Using class weights for classification loss")
        else:
            cls_criterion = torch.nn.CrossEntropyLoss(reduction='none')
            print("No class weights provided, using standard CrossEntropyLoss")
        ...
        for epoch in range(self.args.n_epochs):
            ...
            cls_pred_labels, exp_pred_labels, _, _, valid_loss = self.predict(
                valid_input_ids, valid_attention_masks, valid_cls_labels, valid_exp_labels,
        ...
        print("+ Training ends!")
        print("+ Load best model {}---valid_loss: {}".format(best_epoch['epoch'], best_epoch['valid_loss']), flush=True)
        self.model.load_state_dict(best_model_state_dict)
        self.model.to(self.args.device)

        train_cls_pred_labels, train_exp_pred_labels, _, _, _ = self.predict(
            train_input_ids, train_attention_masks, train_cls_labels, train_exp_labels,
            cls_criterion, exp_criterion, exp_weight, self.args.test_batch_size)
        ...
        for data, exp, cls_pred, cls_true in zip(train_data, train_exp_pred_labels, train_cls_pred_labels, train_cls_labels):
            ...
            text = data.split(" ")
            min_len = min(len(text) - 1, len(exp))
            masked_words = []
            for i in range(1, min_len):
                if i < len(exp):
                    masked_words.append(text[i] if exp[i] == 1 else "*")
                else:
                    masked_words.append(text[i])
            train_exp_pred_data.append(' '.join(masked_words).strip())

        valid_cls_pred_labels, valid_exp_pred_labels, _, _, _ = self.predict(
            valid_input_ids, valid_attention_masks, valid_cls_labels, valid_exp_labels,
            cls_criterion, exp_criterion, exp_weight, self.args.test_batch_size)
        valid_exp_pred_labels_pooled = utils.max_pooling(valid_exp_pred_labels, valid_tokenized_data_slides, valid_data)

        valid_exp_pred_data = []
        for data, exp in zip(valid_data, valid_exp_pred_labels_pooled):
            text = data.split(' ')
            min_len = min(len(text) - 1, len(exp))
            masked_words = []
            for i in range(1, min_len):
                if i < len(exp):
                    ...
                    masked_words.append(text[i])
            valid_exp_pred_data.append(' '.join(masked_words).strip())

        test_cls_pred_labels, test_exp_pred_labels, _, _, _ = self.predict(
            test_input_ids, test_attention_masks, test_cls_labels, test_exp_labels,
            cls_criterion, exp_criterion, exp_weight, self.args.test_batch_size)
        test_exp_true_labels = utils.max_pooling(test_exp_labels, test_tokenized_data_slides, test_data)
        test_exp_pred_labels_pooled = utils.max_pooling(test_exp_pred_labels, test_tokenized_data_slides, test_data)

        test_exp_pred_data = []
        for data, exp in zip(test_data, test_exp_pred_labels_pooled):
            text = data.split(' ')
            min_len = min(len(text) - 1, len(exp))
            masked_words = []
            for i in range(1, min_len):
        ...
        if test_cls_labels is not None:
            test_cls_labels_list = [int(label) if torch.is_tensor(label) else label for label in test_cls_labels]
        else:
    ...
    def classify(self, new_data):
        ...
        tokenized_data, input_ids, attention_masks, tokenized_data_slides = \
            utils.tokenize_text(self.tokenizer, data, self.tokenizer.pad_token, max_len=self.args.max_len)
        ...
        for prepro_txt, exp_label, exp_prob in zip(data, exp_preds, exp_probs):
            try:
                ...
                end_idx = min(len(text) - 1, len(exp_label), len(exp_prob))
                exp_text_i = ' '.join(text[i] for i in range(1, end_idx) if (exp_label[i] == 1))
                exp_text_masked_i = ' '.join(text[i] if (exp_label[i] == 1) else "*" for i in range(1, end_idx))
                pred_text_prob_i = ' '.join(text[i] + "§§§" + str(exp_prob[i]) for i in range(1, end_idx))
                exp_texts.append(exp_text_i)
                exp_text_masked.append(exp_text_masked_i)
                exp_text_probs.append(pred_text_prob_i)
            except Exception as e:
                print("Exception while processing rationale:", e, flush=True)
        self.model.cpu()
        if torch.cuda.is_available() and str(self.args.device).startswith('cuda'):
            torch.cuda.empty_cache()
        return cls_preds, cls_probs, exp_texts, exp_text_masked, exp_text_probs, exp_preds, exp_probs